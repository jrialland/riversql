...
public class JavaResInfo {
	...
	public JSONObject execute()throws Exception {
		...
		JavaResourceNode funz=(JavaResourceNode)obj;
		final String sql=
			"SELECT  created,last_ddl_time, timestamp, status,DBMS_JAVA.LONGNAME(object_name) FROM sys.all_objects where owner=? and object_type='JAVA RESOURCE' and object_name=?";
		//SELECT  created,last_ddl_time, timestamp, status,DBMS_JAVA.LONGNAME(object_name) FROM sys.all_objects where owner='SYS' and object_type='JAVA SOURCE' and object_name='/28221493_ForEachClass'
